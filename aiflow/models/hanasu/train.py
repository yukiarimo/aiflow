...
class TextAudioSpeakerLoader(torch.utils.data.Dataset):
	...
	def get_audio_text_speaker_pair(self, audiopath_sid_text, language='en'):
		...
		text_norm = cleaned_text_to_sequence(text)  # Bypass phonemizer!
...
def run(rank, n_gpus, hps):
	...
	net_g = SynthesizerTrn(len(symbols), 128, hps.train.segment_size // hps.data.hop_length, n_speakers=hps.data.n_speakers, noise_scale_delta=noise_scale_delta, **hps.model).cuda(rank)
	...
	if net_dur_disc is not None:
		optim_dur_disc = torch.optim.AdamW(net_dur_disc.parameters(), hps.train.learning_rate, betas=hps.train.betas, eps=hps.train.eps)
	else:
		optim_dur_disc = None
...
def train_and_evaluate(rank, epoch, hps, nets, optims, schedulers, scaler, loaders, logger, writers):
	...
	for batch_idx, (x, x_lengths, spec, spec_lengths, y, y_lengths, speakers) in enumerate(loader):
		current_mas_noise_scale = (0.01 - net_g.module.noise_scale_delta * global_step)
