...
class MonoTransformerFlowLayer(nn.Module):
	def __init__(self, channels, hidden_channels, mean_only=False, residual_connection=False):  # According to VITS-2 paper fig 1B set residual_connection=True
		assert channels % 2 == 0, "channels should be divisible by 2"
		super().__init__()
		self.channels = channels
		self.hidden_channels = hidden_channels
...
class ResidualCouplingTransformersBlock(nn.Module):
	def __init__(self, channels, hidden_channels, kernel_size, dilation_rate, n_layers, n_flows=4, gin_channels=0, transformer_flow_type="pre_conv"):
		super().__init__()
		self.channels = channels
		self.hidden_channels = hidden_channels
		self.kernel_size = kernel_size
...
class SynthesizerTrn(nn.Module):
	...
	def __init__(self, n_vocab, spec_channels, segment_size, inter_channels, hidden_channels, filter_channels, n_heads, n_layers, kernel_size, p_dropout, resblock, resblock_kernel_sizes, resblock_dilation_sizes, upsample_rates, upsample_initial_channel, upsample_kernel_sizes, n_speakers=0, gin_channels=0, use_sdp=True, **kwargs):
		...
		self.flow = ResidualCouplingTransformersBlock(inter_channels, hidden_channels, 5, 1, 4, gin_channels=gin_channels, transformer_flow_type=self.transformer_flow_type)
	...
	def forward(self, x, x_lengths, y, y_lengths, sid=None):
		...
		# Completely sever the tensor from PyTorch's tracking history
	...
	def infer(self, x, x_lengths, sid=None, noise_scale=0.5, length_scale=1, noise_scale_w=0.5, min_period_duration=24.0):
		...
		with torch.autocast(device_type=x.device.type):
			...
			# Expand Prior to aligned timeline
			m_p = torch.matmul(attn.squeeze(1), m_p.transpose(1, 2)).transpose(1, 2)
			logs_p = torch.matmul(attn.squeeze(1), logs_p.transpose(1, 2)).transpose(1, 2)

			# Latent Variable Inference (Flow Reverse)
	...
	def voice_conversion(self, y, y_lengths, sid_src, sid_tgt, noise_scale=0.0):
		...
		# Prior -> Target (Add Target Speaker traits)
...
def voice_conversion_inference(model=None, source_wav_path=None, source_speaker_id=0, target_speaker_id=1, device="mps", hps=None, pitch_shift=0):
	...
	# Mel Spectrogram
	mel = mel_spectrogram_torch(audio_norm, data_config.filter_length, data_config.n_mel_channels, data_config.sampling_rate, data_config.hop_length, data_config.win_length, data_config.mel_fmin, data_config.mel_fmax, center=False)
...
def convert_to_single_speaker(ckpt_paths, new_emb_size=1024, output_suffix="_single"):
	...
	if isinstance(ckpt_paths, str):
		ckpt_paths = [ckpt_paths]  # Handle single path or list of paths
	...
	for ckpt_path in ckpt_paths:
		...
		if "model" in checkpoint:
			...
			print("No 'model' key found in checkpoint, skipping.\n")
