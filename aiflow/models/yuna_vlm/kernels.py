...
def nearest_interpolate(x, size=None, scale_factor=None):
	...
	kernel = mx.fast.metal_kernel(name="nearest_interpolation", input_names=["input", "dims"], output_names=["output"], source=source)

	threadgroup = get_optimal_threadgroup(out_w, out_h)
	outputs = kernel(inputs=[x_flat, dims], grid=(out_w, out_h, batch_size * channels), threadgroup=threadgroup, output_shapes=[(batch_size * channels * out_h * out_w)], output_dtypes=[mx.float32])
...
def bicubic_interpolate(x, size=None, scale_factor=None, align_corners=False):
	...
	params = mx.array([scale_h, scale_w, 1.0 if align_corners else 0.0], dtype=mx.float32)
	dims = mx.array([batch_size, channels, in_h, in_w, out_h, out_w], dtype=mx.int32)
	x_flat = x.reshape(-1)
	...
	kernel = mx.fast.metal_kernel(name="bicubic_interpolation", input_names=["input", "dims", "params"], output_names=["output"], source=source, header=header)

	threadgroup = get_optimal_threadgroup(out_w, out_h)
	outputs = kernel(inputs=[x_flat, dims, params], grid=(out_w, out_h, batch_size * channels), threadgroup=threadgroup, output_shapes=[(batch_size * channels * out_h * out_w)], output_dtypes=[mx.float32], )
...
def grid_sample(x, grid):
	...
	outputs = kernel(inputs=[x, grid], template=[("T", x.dtype)], output_shapes=[out_shape], output_dtypes=[x.dtype], grid=(mx.prod(mx.array(out_shape)), 1, 1), threadgroup=(256, 1, 1))
	return outputs[0]
