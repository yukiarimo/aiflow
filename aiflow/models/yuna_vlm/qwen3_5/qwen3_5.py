...
class Model(Qwen3VLModel):
	...
	def get_input_embeddings(self, input_ids=None, pixel_values=None, **kwargs):
		...
		if pixel_values is None:
			...
			return InputEmbeddingsFeatures(inputs_embeds=self.language_model.model.embed_tokens(input_ids))
		...
		inputs_embeds, _ = self.merge_input_ids_with_image_features(hidden_states, inputs_embeds, input_ids, self.config.image_token_index, self.config.video_token_index, )

		if image_grid_thw is not None or video_grid_thw is not None:
			position_ids, rope_deltas = self.language_model.get_rope_index(input_ids, image_grid_thw, video_grid_thw, mask)
	...
	def sanitize(self, weights):
		...
		norm_keys = (".input_layernorm.weight", ".post_attention_layernorm.weight", "model.norm.weight", ".q_norm.weight", ".k_norm.weight", )
