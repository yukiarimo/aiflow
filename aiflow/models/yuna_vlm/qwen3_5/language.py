...
class Qwen3_5RotaryEmbedding:
	def __init__(self, dim, max_position_embeddings=2048, base=10000, mrope_section=[11, 11, 0]):
		self.dim = dim
	...
	def __call__(self, x, position_ids):
		...
		inv_freq_expanded = mx.broadcast_to(self.inv_freq[None, None, :, None].astype(mx.float32), (3, position_ids.shape[1], self.inv_freq.shape[0], 1), )
...
class Qwen3_5Attention(nn.Module):
	def __init__(self, args):
		...
		self.rotary_emb = Qwen3_5RotaryEmbedding(int(self.head_dim * args.rope_parameters["partial_rotary_factor"]), max_position_embeddings=args.max_position_embeddings, base=args.rope_parameters["rope_theta"], mrope_section=args.rope_parameters["mrope_section"], )
...
class Qwen3_5GatedDeltaNet(nn.Module):
	def __init__(self, config):
		...
		if self.num_v_heads % self.num_k_heads != 0:
			raise ValueError(f"num_v_heads ({self.num_v_heads}) must be divisible by num_k_heads ({self.num_k_heads})")
		self.conv_kernel_size = config.linear_conv_kernel_dim
		self.layer_norm_epsilon = config.rms_norm_eps
		self.conv_dim = self.key_dim * 2 + self.value_dim
		self.conv1d = nn.Conv1d(in_channels=self.conv_dim, out_channels=self.conv_dim, bias=False, kernel_size=self.conv_kernel_size, groups=self.conv_dim, padding=0, )
	...
	def __call__(self, inputs, mask=None, cache=None):
		...
		q, k, v = [t.reshape(B, S, h, d) for t, h, d in zip(mx.split(conv_out, [self.key_dim, 2 * self.key_dim], -1), [self.num_k_heads, self.num_k_heads, self.num_v_heads], [self.head_k_dim, self.head_k_dim, self.head_v_dim], )]
		...
		out, state = gated_delta_update(q, k, v, a, b, self.A_log, self.dt_bias, state, mask, use_kernel=not self.training, )
...
class Qwen3_5Model(nn.Module):
	def __init__(self, args):
		...
		self.layers = [Qwen3_5DecoderLayer(args=args, layer_idx=i) for i in range(args.num_hidden_layers)]
		self.norm = nn.RMSNorm(args.hidden_size, eps=args.rms_norm_eps)
		self.ssm_idx = 0
		self.fa_idx = args.full_attention_interval - 1

	def __call__(self, inputs, inputs_embeds=None, mask=None, cache=None, position_ids=None):
		...
		for layer, c in zip(self.layers, cache):
			mask_layer = ssm_mask if layer.is_linear else fa_mask
			h = layer(h, mask_layer, c, position_ids)
...
class LanguageModel(nn.Module):
	...
	def get_rope_index(self, input_ids, image_grid_thw=None, video_grid_thw=None, attention_mask=None):
		...
		if input_ids is not None and (image_grid_thw is not None or video_grid_thw is not None):
			...
			for i, input_ids in enumerate(total_input_ids):
				...
				updated_position_ids = mx.concatenate([position_ids[:, :i, :], new_positions, position_ids[:, i + 1:, :], ], axis=1, )
	...
	def __call__(self, inputs, inputs_embeds=None, mask=None, cache=None, **kwargs):
		...
		if position_ids is None and (rope_mask is None or rope_mask.ndim == 2):
			if ((cache is not None and cache[self.model.fa_idx] is not None and (cache_offset == 0)) or self._rope_deltas is None or cache is None):
				if self._position_ids is not None:
					seq_length = inputs.shape[1]
					position_ids = self._position_ids[:, :, cache_offset:cache_offset + seq_length]
				else:
					position_ids, rope_deltas = self.get_rope_index(inputs, image_grid_thw, video_grid_thw, rope_mask)
