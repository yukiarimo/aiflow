...
class Attention(nn.Module):
	def __init__(self, args):
		...
		self.head_dim = head_dim = getattr(args, "head_dim", args.hidden_size // args.num_attention_heads)
		...
		self.rotary_emb = Qwen3VLRotaryEmbedding(head_dim, max_position_embeddings=args.max_position_embeddings, base=args.rope_theta, rope_scaling=self.rope_scaling)
	...
	def __call__(self, x, mask=None, cache=None, position_ids=None):
		...
		output = scaled_dot_product_attention(queries, keys, values, cache, scale=self.scale, mask=mask)
...
class Qwen3VLModel(nn.Module):
	def __init__(self, args):
		...
		self.layers = [Qwen3VLDecoderLayer(args=args, layer_idx=layer_idx) for layer_idx in range(args.num_hidden_layers)]
		self.norm = nn.RMSNorm(args.hidden_size, eps=args.rms_norm_eps)

	def __call__(self, inputs, inputs_embeds=None, mask=None, cache=None, position_ids=None, visual_pos_masks=None, deepstack_visual_embeds=None):
...
class LanguageModel(nn.Module):
	...
	def __call__(self, inputs, inputs_embeds=None, mask=None, cache=None, visual_pos_masks=None, deepstack_visual_embeds=None, **kwargs):
		...
		if position_ids is None and (mask is None or mask.ndim == 2):
			if ((cache is not None and cache[0] is not None and (cache_offset == 0)) or self._rope_deltas is None or cache is None):
		...
		out = self.model(inputs, cache=cache, inputs_embeds=inputs_embeds, position_ids=position_ids, visual_pos_masks=visual_pos_masks, deepstack_visual_embeds=deepstack_visual_embeds)
