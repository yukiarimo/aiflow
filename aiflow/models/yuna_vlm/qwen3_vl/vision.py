...
class PatchEmbed(nn.Module):
	...
	def __call__(self, hidden_states):
		hidden_states = hidden_states.reshape(-1, self.in_channels, self.temporal_patch_size, self.patch_size, self.patch_size).moveaxis(1, 4)
...
class Attention(nn.Module):
	...
	def __call__(self, x, cu_seqlens, rotary_pos_emb=None):
		...
		for q, k, v in zip(*splits):
			output = mx.fast.scaled_dot_product_attention(q, k, v, scale=self.scale, mask=None)
...
class VisionModel(nn.Module):
	def __init__(self, config):
		...
		self.deepstack_merger_list = [PatchMerger(config=config, use_postshuffle_norm=True) for _ in range(len(config.deepstack_visual_indexes))]
	...
	def fast_pos_embed_interpolate(self, grid_thw):
		...
		for t, h, w in grid_thw_list:
			...
			indices = [(base_h[:, None] + w_idxs_floor[None, :]).flatten(), (base_h[:, None] + w_idxs_ceil[None, :]).flatten(), (base_h_ceil[:, None] + w_idxs_floor[None, :]).flatten(), (base_h_ceil[:, None] + w_idxs_ceil[None, :]).flatten()]
			weights = [((1 - dh)[:, None] * (1 - dw)[None, :]).flatten(), ((1 - dh)[:, None] * dw[None, :]).flatten(), (dh[:, None] * (1 - dw)[None, :]).flatten(), (dh[:, None] * dw[None, :]).flatten()]
		...
		for pos_embed, (t, h, w) in zip(patch_pos_embeds_split, grid_thw_list):
			...
			pos_embed = (pos_embed.reshape(t, h // merge_size, merge_size, w // merge_size, merge_size, feature_dim).transpose(0, 1, 3, 2, 4, 5).reshape(-1, feature_dim))
