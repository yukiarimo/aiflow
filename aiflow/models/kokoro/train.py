...
class EmotionalDataset(Dataset):
	def __init__(self, config, embed_generator):
		...
		self.embeddings = [(embed_generator.get_embedding(item["memory"]), embed_generator.get_embedding(item["input"])) for item in tqdm(self.data, desc="Generating embeddings")]
...
def get_device(config):
	if config["device"] == "mps" and torch.backends.mps.is_available(): return torch.device("mps")
	if config["device"] == "cuda" and torch.cuda.is_available(): return torch.device("cuda")
...
def train(config):
	...
	for epoch in range(config["epochs"]):
		...
		for memory_emb, input_emb, targets in train_bar:
			...
			optimizer.zero_grad()
			predictions_tensor, _ = model(input_emb, memory_emb)
